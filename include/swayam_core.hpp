...
class MutationLease {
    AtomicGuard& guard_;
public:
    explicit MutationLease(AtomicGuard& g) : guard_(g) {
...
// [VENOMICA] Restored Quarantine Defense Logic
...
inline void quarantine(const std::string& mutation_source_path, int signal_num) {
    ...
    std::ofstream log("quarantine.log", std::ios::app);
...
inline bool is_quarantined(const std::string& mutation_source_path) {
    ...
    std::ifstream log("quarantine.log");
    std::string line;
...
// [VENOMICA] Guarded Execution with Fork and Quarantine Shield
inline bool run_guarded_mutation(AtomicGuard& guard, const std::string& mutation_binary, const std::string& mutation_source_path) {
    if (is_quarantined(mutation_source_path)) {
        return false; // Blocks endlessly regenerating crashed payloads
    }
    ...
    if (pid < 0) {
        throw std::runtime_error("[VENOMICA] Fork failed launching mutation sandbox.");
    }
    if (pid == 0) {
        const char* args[] = {mutation_binary.c_str(), nullptr};
        execvp(mutation_binary.c_str(), const_cast<char* const*>(args));
        _exit(127); // Correctly avoids double-flushing buffers
    }
